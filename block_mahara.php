...
class block_mahara extends block_list {
    function init() {
        $this->title = get_string('pluginname','block_mahara') ;
    }
    ...
    function get_content() {
        ...
        if (is_mnet_remote_user($USER)) {
            if (debugging() and !empty($CFG->debugdisplay)) {
                $this->content = new stdClass();
                $this->content->footer = html_writer::tag('span',
                    get_string('error_localusersonly', 'block_mahara'),
                    array('class' => 'error'));
                return $this->content;
            } else {
                return '';
        ...
        if (!is_enabled_auth('mnet')) {
            if (debugging() and !empty($CFG->debugdisplay)) {
                $this->content = new stdClass();
                $this->content->footer = html_writer::tag('span',
                    get_string('error_authmnetneeded', 'block_mahara'),
                    array('class' => 'error'));
                return $this->content;
            } else {
                return '';
        ...
        if (!has_capability('moodle/site:mnetlogintoremote', context_system::instance(), NULL, false)) {
            if (debugging() and !empty($CFG->debugdisplay)) {
                $this->content = new stdClass();
                $this->content->footer = html_writer::tag('span',
                    get_string('error_roamcapabilityneeded', 'block_mahara'),
                    array('class' => 'error'));
                return $this->content;
            } else {
                return '';
        ...
        if ($hosts) {
            foreach ($hosts as $host) {
                $icon  = '<img src="'.$OUTPUT->pix_url('i/'.$host->application.'_host') . '"'.
                         ' class="icon" alt="'.get_string('server', 'block_mahara').'" />&nbsp;';
